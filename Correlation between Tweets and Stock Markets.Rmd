...
```{r}
twt%>%
  select(comment_num, retweet_num, like_num)%>%
  mutate(comment_num=if_else(comment_num==0,"no_value","yes_value"),
         retweet_num=if_else(retweet_num==0,"no_value","yes_value"),
         like_num=if_else(like_num==0,"no_value","yes_value"))%>%
  gather(variable,value)%>%
  group_by(value, variable) %>%
  summarize(p=sprintf('%.1f%%',n()*100/nrow(twt)), .groups="drop")%>%
  pivot_wider(id_cols=value, names_from = variable, values_from=p)%>%
  rename("yes/no values"=value)
```
...
```{r}
twt$body_len=sapply(lapply(strsplit(twt$body," "), function(x){x[!x==""]}), length)
