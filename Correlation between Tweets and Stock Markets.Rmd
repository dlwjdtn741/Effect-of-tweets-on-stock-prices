...
Also, the data set should have `r format(sum(rf_cmpy$Referred_Company_Count_per_Tweet*rf_cmpy$n_occurrence), big.mark=",")` rows but the table actually has `r format(nrow(twt_to_cmpy),big.mark=",")`.
...
twt_w %>%
  select(writer, tweet_ct, p_tweet_ct) %>%
...
twt_w %>%
  select(writer, comment_num, p_comment_num) %>%
...
twt_w %>%
  select(writer, like_num, p_like_num) %>%
...
twt_w %>%
  select(writer, retweet_num, p_retweet_num) %>%
